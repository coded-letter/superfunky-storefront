...
import { Seo, SocialFeedGrid, useLanguage, useT } from "@funky/ui";
...
export function CommunityTagArchivePage() {
  ...
  if (isLoading) return <ContentLoadingState label={t("loading.community_tag")} />;
  if (error) {
    return <ArchiveDirectoryStatus title={t("error.community_tag_unavailable")} message={error.message} href="/community-tag" linkLabel={t("archive.browse_community_tags")} isError />;
  }
  ...
  const description = t("archive.community_tag_description", { count: tag.postCount, name: tag.name });

  return (
    <div className="grid gap-8">
      <Seo
        title={t("archive.community_tag_title", { name: tag.name })}
      ...
      <Breadcrumbs
        items={[
          ...
          { label: t("archive.community"), href: "/community" },
          { label: t("archive.community_tags_title"), href: "/community-tag" },
          { label: `#${tag.name}` },
        ]}
      />
      <header className="grid gap-3 rounded-3xl border border-zinc-200/80 bg-gradient-to-br from-brand-50 to-white p-8 shadow-soft dark:border-zinc-800 dark:from-brand-950/30 dark:to-zinc-950 sm:p-10">
        <span className="text-xs font-semibold uppercase tracking-[0.28em] text-brand-600 dark:text-brand-400">{t("archive.community_tag_kicker")}</span>
      ...
      <SocialFeedGrid
        title={t("archive.tagged_title", { name: tag.name })}
        subtitle={t("archive.community_tag_summary", {
          posts: posts.length,
          profiles: new Set(posts.map((post) => post.author.handle)).size,
        })}
        posts={posts}
        pageSize={12}
