...
test("community members use an exact comma-separated WordPress role whitelist", () => {
  ...
  assert.match(storefrontShortcodes, /communityMemberMatchesRoles\(member\.memberTypes, requestedRoles\)/);
...
test("community role matching tolerates legacy cache entries and fails closed", () => {
  assert.deepEqual(normalizeCommunityMemberTypes(undefined), []);
  assert.deepEqual(normalizeCommunityMemberTypes(null), []);
  assert.deepEqual(normalizeCommunityMemberTypes([" Member ", "administrator", "member", 42]), ["member", "admin"]);
  assert.equal(communityMemberMatchesRoles(undefined, []), false);
  assert.equal(communityMemberMatchesRoles(undefined, ["member"]), false);
  assert.equal(communityMemberMatchesRoles(["administrator"], ["admin"]), true);
  assert.equal(communityMemberMatchesRoles(["customer"], ["member"]), false);
  assert.equal(communityMemberMatchesRoles(["member"], []), true);
});
