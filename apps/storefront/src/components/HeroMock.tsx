...
export type HeroMockProps = {
  ...
  title: string;
  description?: string;
  /** Background/side image — required for the `fullbleed` and `split` variants. */
  image?: string;
  primaryCta?: HeroCta;
  secondaryCta?: HeroCta;
...
export function HeroMock({
      ...
      <SliderMock
        ...
        renderItem={(slide, index) => (
          <HeroMock
            ...
            headingLevel={index === 0 ? headingLevel : "h2"}
            height="100%"
          />
        )}
      />
  ...
  if (variant === "glow") {
    return (
      <section
        className={`funky-hero funky-hero--glow relative overflow-hidden bg-zinc-900 px-6 py-14 sm:px-12 sm:py-20 dark:bg-zinc-900 ${
        ...
        <div className="relative mx-auto grid w-full max-w-2xl gap-5 px-2 text-center sm:px-6">
          ...
          {primaryCta || secondaryCta ? (
            <div className="mx-auto flex flex-wrap justify-center gap-3 pt-2">
              {primaryCta ? (
                <Link to={primaryCta.href} className="rounded-full bg-brand-gradient px-6 py-3 text-sm font-semibold text-white no-underline transition hover:-translate-y-0.5">
                  {primaryCta.label}
                </Link>
              ) : null}
              {secondaryCta ? (
                <Link to={secondaryCta.href} className="rounded-full border border-white/25 px-6 py-3 text-sm font-semibold text-white no-underline transition hover:bg-white/10">
                  {secondaryCta.label}
                </Link>
              ) : null}
            </div>
  ...
  if (variant === "fullbleed") {
    return (
      <section
        className={`funky-hero funky-hero--fullbleed relative flex items-end overflow-hidden shadow-soft-lg ${
          fullWidth ? breakoutClassName : "rounded-3xl"
      ...
      >
        ...
        <div
          ...
          style={{ backgroundColor: "rgb(var(--brand-600) / 0.16)" }}
          aria-hidden="true"
        />
        <div
        ...
        >
          <div className="relative grid max-w-xl gap-4 p-8 sm:p-12">
            {kicker ? (
            ...
            {primaryCta || secondaryCta ? (
              <div className="flex flex-wrap gap-3 pt-2">
                {primaryCta ? (
                  <Link to={primaryCta.href} className="rounded-full bg-brand-gradient px-6 py-3 text-sm font-semibold text-white no-underline shadow-glow transition hover:-translate-y-0.5">
                    {primaryCta.label}
                  </Link>
                ) : null}
                {secondaryCta ? (
                  <Link to={secondaryCta.href} className="rounded-full border border-white/40 px-6 py-3 text-sm font-semibold text-white no-underline transition hover:bg-white/10">
                    {secondaryCta.label}
                  </Link>
                ) : null}
              </div>
  ...
  if (variant === "split") {
    return (
      <section
        className={`funky-hero funky-hero--split grid gap-0 overflow-hidden border border-zinc-200/80 bg-white shadow-soft-lg dark:border-zinc-800 dark:bg-zinc-900 sm:grid-cols-2 ${
          fullWidth ? breakoutClassName : "rounded-3xl"
        }`}
        style={heightStyle}
      >
  ...
  if (variant === "minimal") {
    return (
      <section
        className={`funky-hero funky-hero--minimal relative grid gap-5 overflow-hidden border border-zinc-200/80 bg-gradient-to-b from-zinc-50 to-white px-6 py-14 text-center shadow-soft dark:border-zinc-800 dark:from-zinc-900/60 dark:to-zinc-950 sm:px-12 sm:py-20 ${
          fullWidth ? breakoutClassName : "rounded-3xl"
      ...
      >
        ...
        <div className="mx-auto grid max-w-2xl justify-items-center gap-4">
          ...
          {primaryCta || secondaryCta ? (
            <div className="mx-auto flex flex-wrap justify-center gap-3 pt-2">
              {primaryCta ? (
                <Link to={primaryCta.href} className="rounded-full bg-zinc-900 px-6 py-3 text-sm font-semibold text-white no-underline transition hover:-translate-y-0.5 hover:shadow-soft dark:bg-zinc-100 dark:text-zinc-900">
                  {primaryCta.label}
                </Link>
              ) : null}
              {secondaryCta ? (
                ...
                >
                  {secondaryCta.label} <ArrowRight className="h-4 w-4" aria-hidden="true" />
                </Link>
              ) : null}
            </div>
          ) : null}
  ...
  // variant === "strip"
  return (
    <section className="funky-hero funky-hero--strip flex flex-col items-center justify-between gap-4 rounded-2xl bg-brand-gradient px-6 py-5 shadow-glow sm:flex-row sm:px-8">
      ...
      {primaryCta ? (
        <Link
          to={primaryCta.href}
          className="inline-flex shrink-0 items-center gap-1.5 rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-brand-700 no-underline transition hover:-translate-y-0.5 hover:shadow-soft"
        >
