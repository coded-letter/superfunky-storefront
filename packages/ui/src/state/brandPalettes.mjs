 ...
 */
export const BRAND_GRADIENT_STYLE_OPTIONS = [
  { value: "gradient", label: "Gradient" },
  { value: "flat", label: "Flat" },
];

export const BRAND_COLOR_STEPS = ["50", "100", "200", "300", "400", "500", "600", "700", "800", "900", "950"];
...
export const BRAND_PALETTE_OPTIONS = Object.keys(BRAND_PALETTES)
...
export function brandPaletteCssVariables(paletteId, gradientStyle = "gradient") {
  const palette = BRAND_PALETTES[paletteId] ?? BRAND_PALETTES.violet;
  const declarations = BRAND_COLOR_STEPS.map(
    (step) => `--brand-${step}:${hexToRgbTriplet(palette.scale[step])}`,
  );
  const flat = gradientStyle === "flat";
  const gradientFrom = flat ? palette.scale["600"] : palette.gradientFrom;
  const gradientTo = flat ? palette.scale["600"] : palette.gradientTo;
  declarations.push(`--brand-gradient-from:${hexToRgbTriplet(gradientFrom)}`);
  declarations.push(`--brand-gradient-to:${hexToRgbTriplet(gradientTo)}`);
  return declarations;
}

export function applyBrandPalette(paletteId, gradientStyle = "gradient") {
  if (typeof document === "undefined") return;
  const root = document.documentElement;
  for (const declaration of brandPaletteCssVariables(paletteId, gradientStyle)) {
